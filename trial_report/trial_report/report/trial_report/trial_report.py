...
def execute(filters=None):
	...
	columns = get_columns()
	# print("initial check dat",data)
	# print("initial check",columns)
	# print("Inside") 
	return columns, data
# print("Outside")
# print("second check",data)
# print("second check",columns)
...
def get_data(filters):
	...
	print("company currency",company_currency)

	if not accounts:
		return None

	accounts, accounts_by_name, parent_children_map = filter_accounts(accounts)
	# print("accounts value",accounts)
	# print("accounts_by_name value",accounts_by_name)
	# print("parent_children_map value",parent_children_map)
	...
	print('min left & max right',min_lft,max_rgt)
	gl_entries_by_account = {}

	print('opening_balances1',gl_entries_by_account)
	...
	print('opening_balances 2',gl_entries_by_account)
	...
	print("all account data from account doctype",data)
...
@frappe.whitelist()
def upload_chart_of_accounts(name):
	global data
	print("\n\n\n\n\n\nupload\n\n\n\n\n")
	print("This is calling function")
	print("name", name)
	...
	try:
		...
		for row in sheet.iter_rows(min_row=2, values_only=True):  
			...
			for data_account in data:
				if 'account' in data_account:
					account_data = data_account.get('account')
					print("account name from data dict", account_data)
					print("account name from sheet", account)
					if account_data == account:
						data_account['opening_debit'] = opening_dr
						data_account['opening_credit'] = opening_cr
						data_account['debit'] = debit
						data_account['credit'] = credit
						data_account['closing_debit'] = closing_dr
						data_account['closing_credit'] = closing_cr
						found = True
						break
		...
		accounts = frappe.get_all('Account', fields=['name', 'account_name', 'parent_account', 'account_type', 'root_type', 'is_group'])
		# print("get all accounts data",accounts)
		for account in accounts:
			print(f"Account Name: {account['account_name']}, Parent Account: {account['parent_account']}, Account Type: {account['account_type']}, Root Type: {account['root_type']}, Is Group: {account['is_group']}")


		return {'status': 'success', 'message': 'Chart of accounts imported successfully'}
	except Exception as e:
		frappe.log_error(frappe.get_traceback(), "Chart of Accounts Import Failed")
